class CLI
    ...
    def show_menu
      while true
        choice = single_select("What would you like to do?", MAIN_MENU)
        case choice
          when "Exit Flix-Me"
            return
          when "Friends"
    ...
    def recommend_a_movie
      choice = single_select("Who's watching?", RECOMMEND_MENU)
      case choice
        when "Just for me"
          multilple_recomendation
        when "With friends"
          friends_names = multi_select("Select one or more of your friends:",  @user.get_friends_names)
          multilple_recomendation(friends_names)
      end
    end

    def multilple_recomendation(friends = [])
      type_choice = single_select("Select one of the following criteria:", RECOMMEND_MENU_REFINED)
      if friends.size > 0 #with friend
        case type_choice
          when "Top Movies"
            puts  @user.get_top_rated_recommendations(friends)
          when "By Genres"
            genre = single_select("Select one of the following genre:", GENRE_MENU)
    ...
    def show_movie_reviews
      ...
      case movies.size
        ...
        when 1
          ...
          choice = single_select("Multiple movies found, select one of the following:", movies.map(&:title))
    ...
    def update_existing_review
      ...
      new_review = ask("Type a new review for the movie: #{movie_name}")
    ...
    def get_movies_by_name
      target_title = ask("What's the name of the movie?")
    ...
    def write_review(choice)
      ...
      target_comment = ask("Any additional comment?")
    ...
    def friend_list_operations
      choice = single_select("Select one of the following:", FRIENDLIST_OPTIONS)
      case choice
        when "Add new Friend"
          add_new_friend
    ...
    def friend_reviews
      temp_target_friend = single_select("Select one of your friends:", @user.get_friends_names), "\n#{temp_target_friend} has the following reviews:"
      puts User.find_by(name: temp_target_friend).format_all_reviews
    end

    def delete_friend
      temp_target_friend = normalizer(ask("Who you want to delete from your Friendlist?"))
    ...
    def show_their_friends
      ...
      puts @user.get_friends_names #refracted
    end
    ...
    def fun_facts
      choice = single_select("Select one of the following:", FUN_FACTS)
      case choice
